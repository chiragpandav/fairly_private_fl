...
class ADULT(BaseDataset):
    ...
    def __init__(self, name='ADULT', single_bit_binary=False, device='cpu', random_state=42,name_state=0):
        ...
        test_data_df = pd.read_csv(f'clients_data/raw_data/client_credit.test', delimiter=',', names=list(self.features.keys()), skiprows=1, engine='python')

        # train_data_df = pd.read_csv('datasets/ADULT/credit.data', delimiter=',', names=list(self.features.keys()), engine='python')
        # test_data_df = pd.read_csv('datasets/ADULT/credit.test', delimiter=',', names=list(self.features.keys()), skiprows=1, engine='python')
